...
/**
 *
 * http://www.cnblogs.com/xbq8080/p/6770077.html
 *
...
public class ZookeeperLock implements Watcher{
    ...
    private static String CONNECT_SERVER = "192.168.0.65:2182";
    // session过期时间
    ...
    public void lock(){
        ...
        if(tryLock()){
            // 获取 成功
            logger.info("Thread" + Thread.currentThread().getName() + " -- hold lock!");
            return;
        }
        // 等待并获取锁
        try {
    ...
    public void unLock(){
        try {
            ...
            zk.close();
            logger.info("Thread" + Thread.currentThread().getName() +" unlock success! 锁节点：" + myzkNode);
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (KeeperException e) {
            e.printStackTrace();
