package com.lami.mina.framepackage.mina;
...
public class ConnectionManager {
    ...
    private class DefaultHandler extends IoHandlerAdapter {
        ...
        public void messageReceived(IoSession session, Object message) throws Exception {
            logger.info("收到数据，接下来你要怎么解析数据就是你的事了");
    ...
    private  class MyIoFilterAdapter extends IoFilterAdapter {
        ...
        public void sessionClosed(NextFilter nextFilter, IoSession session) throws Exception {
            logger.info("连接关闭，每隔5秒进行重新连接");
            for(;;){
                if(mConnection==null){
                    break;
                }
                if(ConnectionManager.this.connect()){
                    logger.info("断线重连[" + mConnection.getDefaultRemoteAddress().getHostName() + ":" +
